...
| V14 | Bridged fraction for studs and rafters taken as member width ÷ spacing | `apps/web/src/state/model.ts` `bridgedPercentFromDimensions` | Whether BR 443 prescribes how the bridged fraction of a timber-frame wall is derived, and whether it gives a standard allowance to use instead. The division itself is arithmetic and needs no standard; what needs checking is **what to divide**. The figure covers the repeating members only — sole and head plates, noggins, lintels and doubled studs at openings are extra, so a whole-element allowance is larger. The UI says so and offers a typed percentage as the alternative | Under-reports bridging, so under-reports the U-value, wherever a user takes the derived figure as the whole allowance |
