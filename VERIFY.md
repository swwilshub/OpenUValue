...
| # | Item | Where | What to check | If wrong |
|---|---|---|---|---|
| V4 | **Unventilated air layer resistance table** — every value, for all three heat-flow directions | `engine/src/constants.ts` `UNVENTILATED_AIR_LAYER_TABLE` | All 27 tabulated values against a printed copy. Also: BS EN ISO 6946:2007 presents these as a table, while the 2017 edition moved to a calculation procedure for air layers in an annex. Confirm which basis BR 443 UK work should follow, and whether the tabulated values are reproduced unchanged in 2017 | Changes every build-up containing a cavity **Partly confirmed:** BR 443 (2019) 4.7.1 gives R = 0.18 m²K/W for an unventilated masonry cavity with normal emissivity, which is this table's horizontal value at 25 mm and above. That is one of its 27 entries; the rest are still unchecked |
...
| V22 | The ΔU<sub>g</sub> scaling by (R<sub>insulation</sub> / R<sub>total</sub>)² | `engine/src/corrections.ts` `computeCorrections` | The squared-ratio scaling and its formula number in BS EN ISO 6946 Annex F. **BR 443 (2019) 4.8.1 gives the three ΔU″ levels (0.00, 0.01, 0.04) and those are attributable; the scaling is not** — the annex is not in the freely published preview. Where no insulation layer can be identified the unscaled figure is used instead, with a warning, which over-estimates | Over- or under-states the air gap correction. The 3 % threshold means it often makes no difference to the reported U-value |
...
| V24 | **The approximate fastener procedure, BS EN ISO 6946 Annex F.3.2**, is not implemented — only BR 443 4.8.3's detailed route (ΔU<sub>f</sub> = χ · n<sub>f</sub>) | `engine/src/fasteners.ts` | Whether the annex's approximate formula can be obtained and implemented. Searched and not found in: BR 443 (2019) 4.8.3, BR 443 (2006) 4.9.3, the ISO 6946:2017 published preview, the ISO 6946:2007 published preview, and the Home Energy Model technical documentation — every one of them points at the annex without reproducing it. **The detailed route is not in doubt**: BR 443 4.8.3(a) states it in one sentence and the units confirm it (W/K per fastener × fasteners/m² = W/(m²·K)) | A user without a χ for their fixing gets no fastener correction at all, so their U-value is better than the real one. The UI says so explicitly rather than letting a missing correction read as zero |
