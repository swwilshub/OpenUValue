...
| # | Item | Where | What to check | If wrong |
|---|---|---|---|---|
| V4 | **Unventilated air layer resistance table** | `engine/src/constants.ts` `UNVENTILATED_AIR_LAYER_TABLE` | **Two separate questions, and only one of them is still open.** (a) *Are our numbers the 2007 table's numbers?* Largely yes: **10 of the 27 entries are now independently confirmed** against Malta's Technical Document F Part 1 (May 2023) §8, a public government document whose air-space rows cite EN ISO 6946 — 5 mm 0.11; 10 mm 0.15; 15 mm upward 0.16 and horizontal 0.17; 25 mm 0.16 / 0.18 / 0.19; 50 mm 0.16 / 0.18 / 0.21 — plus BR 443 (2019) 4.7.1's 0.18 for a masonry cavity. Matched on each row's *description* rather than its thickness column, which carries obvious typos (15 mm listed as d = 0.017, 5 mm as d = 0.05). **One row disagrees**: it gives a 35 mm floor cavity 0.17, where our table interpolates ~0.198, and it is inconsistent with that document's own 25 mm floor value of 0.19 — treated as an error in the secondary source, not acted on. (b) *Is the 2007 table the right basis at all?* **No** — ISO/DIS 6946:2015 Annex D replaces it with R<sub>a</sub> = 1/(h<sub>a</sub> + h<sub>r</sub>), h<sub>r</sub> depending on surface emissivity and h<sub>a</sub> on pitch and ΔT. Migrating to that calculation is the real work here | Our cavity resistances are the 2007 basis and are now largely verified *as that*. The 2017 calculation is emissivity- and pitch-aware where the table is not, so a reflective or pitched cavity is the case where the two diverge |
...
| V27 | **Air voids are partly distinguished from air layers** | `engine/src/resistance.ts` `AIR_LAYER_MAX_THICKNESS_TO_WIDTH_RATIO` | **Advanced, not closed.** BR 443 (2006) 4.8.1 gives the UK test verbatim: "An airspace for which the thickness (in the heat flow direction) is less than one-tenth of its width or height is also treated as an air layer; examples include the space between the battens in a dry-lined wall, or the space under tiles on a pitched roof." A bridged cavity is now checked against it and warns when it fails — so the common cases the old row worried about (battens, dabs) are settled *as air layers*, which is the opposite of what that row assumed. **Two things still open.** (a) BR 443's wording is "width **or** height", and a batten running the full storey leaves a tall pocket, so a pocket failing on width may pass on height; we test width only and say so in the warning rather than declaring the layer wrong. (b) When the test genuinely fails, ISO/DIS 6946:2015 Annex D.4's separate air-void calculation is still not implemented — we warn instead of computing it | A cavity divided into genuinely small pockets still gets a layer's resistance, which over-states it. The warning now says so at the point it happens |
