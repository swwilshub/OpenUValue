...
| V34 | **Wind-driven rain exposure zones and what they allow** | `engine/src/exposure.ts` | The four band boundaries — sheltered under 33, moderate 33 to under 56.5, severe 56.5 to under 100, very severe 100 or more litres/m² per spell — are read off the Approved Document C exposure figure, whose categories come from BS 8104. **Read off the figure, not from the document's text**, which I could not obtain: both the gov.uk and LABC copies 404'd. **What is not implemented:** Approved Document C's per-zone construction table, which says which wall build-ups it accepts in which zone and at what minimum cavity width. Only the headline rule is applied, that fair-faced masonry with a fully filled cavity is not accepted in the very severe zone. The severe zone's advice therefore tells the reader to check the document rather than pretending to know | A build-up that the Approved Document restricts in the severe zone gets advice to go and check rather than a verdict. Nothing here changes a calculated number — the zone drives guidance only |
| V35 | **The grid map of where each exposure band falls** | `apps/web/src/components/ExposureMap.tsx` `PATTERN` | A nine-by-thirteen grid of squares standing in for the UK, each square carrying one of the four bands. **It is not traced from, derived from, or checked against the Approved Document C figure** — it is drawn from the plain-English `where` line already on each band in `exposure.ts` (western coasts and high ground wet, central and eastern England dry), at a resolution far too coarse to read a boundary off. Clicking it sets the band and the panel then says the band was suggested rather than read. **What to check:** whether the general pattern is even roughly right for the places a reader will recognise, and whether the suggestion wording is strong enough that nobody treats a square as a lookup | Someone could take a suggested band as their zone and, in the worst case, fill a cavity the Approved Document would not let them fill. Mitigated three ways: the suggestion is labelled as one, the coarseness is visible, and the full-fill conflict warning fires on the band actually selected. Nothing here changes a calculated number |
