...
| V35 | **The four colours the map reader matches against** | `apps/web/src/state/exposureColour.ts` `DIAGRAM_12_ZONE_COLOURS` | The zone is read off the user's own copy of Diagram 12 by the shade under the pointer, which needs four RGB values and no geography at all. They were sampled from the figure's own key rendered at 300 dpi: zone 1 (237, 237, 238), zone 2 (218, 219, 219), zone 3 (197, 198, 200), zone 4 (178, 179, 181). The map body uses the same four, which is what makes the reading possible. A click is voted on a 5 × 5 window, white is reported as sea, near-black as a coastline or a label, and a shade more than 14 from every key colour as unreadable. **What to check:** whether another rendering of the figure — a different PDF viewer, a scanned copy, a screenshot through a colour-managed display — shifts the greys beyond the 14 tolerance, in which case the reader says it cannot tell rather than guessing. The map itself is Crown copyright and is deliberately not shipped: its notice allows free reproduction only for research, private study or internal circulation | A misread band would give the wrong cavity-fill advice. The failure mode is designed to be a refusal rather than a wrong answer, and picking a band by hand always overrides it |
